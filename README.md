# study-portfolio

무무(mumu)의 학습 관제 센터 — 공부한 모든 것(PQC/암호학, 화이트햇스쿨, 논문, 삽질)을 기록하는
터미널 감성 정적 사이트.

## 구조

```
data/           모든 기록 (YAML / 마크다운) — 사람이 편집하는 곳
  weeks/        주간 로그 (YYYY-Www.yml)
  notes/        위키 노트 (마크다운, [[링크]] 문법)
  papers.yml    논문 리딩 노트
  projects.yml  프로젝트 (public: false 면 렌더링 제외)
  fails.yml     삽질/실패 기록
  skills.yml    스킬 트리
  me.yml        취향 코너
  site.yml      닉네임·컬러·연락처 등 전역 설정
scripts/build.py  data/ → docs/ 정적 사이트 생성기
templates/      주간 기록 템플릿
assets/         CSS/JS 원본 (빌드 시 docs/assets 로 복사)
docs/           빌드 결과물 = GitHub Pages 배포 대상 (직접 수정 금지)
```

## 옵시디언으로 노트 쓰기

**볼트 = `data/notes/`** — 옵시디언에서 이 폴더를 열면 곧바로 저장소 안에서 작업하게 되므로
따로 옮기거나 복사하는 단계가 없다.

처음 한 번만 설정:

1. 옵시디언 → `Open folder as vault` → `~/study-portfolio/data/notes` 선택
2. 설정 → 파일 및 링크 → **첨부 파일 기본 위치**를 `_attachments` 폴더로 지정
3. 설정 → 파일 및 링크 → **내부 링크를 항상 업데이트** 켜기

그다음은 평소대로 쓰면 된다. 사이트가 알아서 처리하는 것들:

| 옵시디언에서 | 사이트에서 |
|---|---|
| 파일명에 공백·한글·괄호 | 슬러그로 변환 (`PE 파일 구조.md` → `pe-파일-구조.html`) |
| `[[노트 이름]]` | 링크 (파일명·제목 아무거나 매칭) |
| `[[노트\|다르게 보이기]]` | 별칭 링크 |
| `[[노트#소제목]]` | 해당 소제목으로 점프 |
| `![[이미지.png]]` | 이미지 복사 + 삽입 |
| `> [!note] 제목` | 인용구 |
| 하위 폴더 정리 | 그대로 동작 (주소는 평평하게) |
| 본문 맨 위 `# 제목` | 중복이라 자동 제거 |

**초고는 `draft: true`** — frontmatter에 이 줄이 있으면 사이트에 안 나간다. 다 쓰고 지우면 공개된다.
`templates/note-template.md` 를 옵시디언 템플릿으로 등록해두면 편하다.

`_` 나 `.` 으로 시작하는 폴더는 사이트가 무시한다. 공개하기 싫은 메모는 `_비공개/` 같은 폴더에 두면 되지만,
**이 저장소는 공개이므로 파일 자체는 깃허브에 올라간다.** 진짜 남에게 보이면 안 되는 건 여기 두지 말 것.

## 사이트에 반영

```bash
./publish.sh              # 빌드 + 커밋 + 푸시
./publish.sh "메시지"      # 커밋 메시지 지정
```

첫 실행 때 빌드용 `.venv` 를 자동으로 만든다. 반영까지 1분쯤 걸린다.

## 주간 루틴 (15분 상한)

1. `cp templates/week-template.yml data/weeks/2026-W32.yml` — 이번 주차로 복사
2. 목록만 채운다. 문장 다듬지 않기. **15분 넘기지 않기**
3. `python3 scripts/build.py`
4. `git add -A && git commit -m "log: 2026-W32"` → push

커밋 히스토리가 곧 꾸준함의 증거다. 주 1회 이상 커밋이 목표.

## 커밋 전 공개 체크리스트

- [ ] 화이트햇스쿨 **내부 자료/교육 자료** 포함되지 않았는가?
- [ ] **비공개 취약점**, 제보 전 취약점 정보가 없는가?
- [ ] 타인의 **개인정보/연락처**가 없는가?
- [ ] 팀 프로젝트 내용은 팀원 동의를 받았는가? (애매하면 `public: false`)

## 배포 (GitHub Pages)

저장소 Settings → Pages → Source: `Deploy from a branch`, Branch: `main`, Folder: `/docs`

## 이스터에그

사이트 어딘가에 암호 퍼즐이 있다. 스포일러는 이 README에도 안 적는다.
